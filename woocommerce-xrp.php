...
function wc_gateway_xrp_checkout_handler()
{
    ...
    $account = $x_account = trim(
        get_option('woocommerce_xrp_settings')['xrp_account']
    );

    $url = sprintf(
        'https://xrpaddress.info/api/encode/%s/%d',
        $account,
        (int)$order->get_meta('destination_tag', true)
    );

    if (($x_account = get_transient($url)) === false) {
        $res = wp_remote_get($url);
        if (!is_wp_error($res) && $res['response']['code'] === 200 &&
        (($json = json_decode($res['body'])) !== null) &&
        isset($json->address)) {
            $x_account = $json->address;
            set_transient($url, $x_account, 3600);
        }
    }
