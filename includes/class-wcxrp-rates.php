...
class WCXRP_Rates
{
    ...
    {
        $url = 'https://api.bittrex.com/api/v1.1/public/getticker?market=USD-XRP';
        $res = wp_remote_get($url);
        if (is_wp_error($res) || $res['response']['code'] !== 200) {
            return false;
        }
    ...
    private function bitmex()
    {
        ...
        $url = 'https://www.bitmex.com/api/v1/orderBook/L2?symbol=xrp&depth=1';
        $res = wp_remote_get($url);
        if (is_wp_error($res) || $res['response']['code'] !== 200) {
            return false;
        }
    ...
    {
        $url = 'https://www.bitrue.com/kline-api/publicXRP.json?command=returnTicker';
        $res = wp_remote_get($url);
        if (is_wp_error($res) || $res['response']['code'] !== 200) {
            return false;
        }
    ...
    /**
     * Get Exchange rate from okex
     * @return bool|float|int
     */
    private function okex()
    {
        ...
        return $this->to_base($rate->last, 'USD');
    }
